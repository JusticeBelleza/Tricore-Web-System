...
import { 
  ...
  ChevronDown, DollarSign, CreditCard, FileText, Calendar, ShieldAlert
} from 'lucide-react';
...
export default function AdminOrders() {
    ...
    try {
      const [ordersRes, fleetRes, driversRes] = await Promise.all([
        supabase.from('orders').select(`
          *, 
          companies ( name, address, city, state, zip, email, phone ), 
        ...
        supabase.from('user_profiles').select('id, full_name').eq('role', 'driver').order('full_name', { ascending: true }) 
      ]);
  ...
  const confirmAssignment = async (e) => {
    ...
    try {
      // 🚀 FIXED: Removed `vehicle_type` to prevent the 400 Database Schema Error
      const updates = { 
                              ...
                              <div className="lg:col-span-2 space-y-6">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                  <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
                                    ...
                                    {order.company_id ? (
                                      <div className="text-sm font-medium text-slate-700 space-y-1">
                                        <p className="font-bold text-slate-900 text-base">{order.companies?.name}</p>
                                        <p>{order.companies?.address || 'No address provided'}</p>
                                        {order.companies?.city && <p>{order.companies.city}, {order.companies.state} {order.companies.zip}</p>}
                                      </div>
                                    ) : (
                                      <div className="text-sm font-medium text-slate-700 space-y-1">
                                        <p className="font-bold text-slate-900 text-base">{order.shipping_name}</p>
                                        <p>Retail Purchase</p>
                                      </div>
                                    )}
                                  </div>
                                  ...
                                  <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
                                    ...
                                    <div className="text-sm font-medium text-slate-700 space-y-1">
                                      <p className="font-bold text-slate-900 text-base">{order.shipping_name}</p>
                                      <p>{order.shipping_address || <span className="text-red-500 italic">No shipping address</span>}</p>
                                ...
                                {(order.status === 'ready_for_delivery' || order.status === 'shipped' || order.status === 'delivered') && order.driver_name && (
                                  <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm space-y-4">
                                    ...
                                    <div className="space-y-3 text-sm">
                                      <div><p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Driver</p><p className="font-bold text-slate-900 flex items-center gap-1.5"><User size={14} className="text-slate-400"/> {order.driver_name}</p></div>
                                      <div><p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Vehicle</p><p className="font-medium text-slate-700 flex items-center gap-1.5"><Car size={14} className="text-slate-400"/> {order.vehicle_name} ({order.vehicle_type})</p></div>
                                      <div><p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">License Plate</p><p className="font-mono font-bold text-slate-700 flex items-center gap-1.5"><Hash size={14} className="text-slate-400"/> {order.vehicle_license}</p></div>
                                    </div>
                                  </div>
                                )}
                              </div>
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg flex flex-col border border-slate-100 overflow-hidden">
            ...
            <form onSubmit={confirmAssignment} className="p-6 space-y-5 max-h-[75vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div><label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1.5">Vehicle Name</label><input type="text" required value={vehicleName} onChange={(e) => setVehicleName(e.target.value)} className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-slate-900 text-sm font-medium transition-all" /></div>
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1.5">Vehicle Type</label>
                  <select required value={vehicleType} onChange={(e) => setVehicleType(e.target.value)} className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-slate-900 text-sm font-medium cursor-pointer transition-all">
                    <option value="Car">Car</option><option value="Cargo Van">Cargo Van</option><option value="Box Truck">Box Truck</option><option value="Other">Other</option>
                  </select>
                </div>
              </div>
