...
export default function Catalog() {
  ...
  const openProductModal = (familyName, familyProducts) => {
    ...
    const defaultVariants = globalVariants.filter(v => v.product_id === defaultProduct.id);
  ...
  const handleSizeChange = (newProductId) => {
    ...
    const newVariants = globalVariants.filter(v => v.product_id === newProductId);
  ...
  const handleAddToCart = () => {
    ...
    const variant = globalVariants.find(v => v.id === selectedVariantId);
  ...
  const activeVariants = activeProduct ? globalVariants.filter(v => v.product_id === activeProduct.id) : [];
  const activeVariant = activeVariants.find(v => v.id === selectedVariantId) || activeVariants[0];
  const { finalPrice: displayPrice } = getVariantPrice(activeProduct, activeVariant);
          ...
          <div className="space-y-4 sm:space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-6">
              {displayedFamilies.map(([familyName, familyProducts]) => (
                <ProductFamilyCard key={familyName} familyName={familyName} familyProducts={familyProducts} globalVariants={globalVariants} getVariantPrice={getVariantPrice} onClick={() => openProductModal(familyName, familyProducts)} />
              ))}
            </div>
